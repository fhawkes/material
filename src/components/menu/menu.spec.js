ddescribe('md-menu directive', function() {
  ...
  it('removes everything but the first element', function() {
    ...
    expect(menu.firstElementChild.nodeName).toBe('LABEL');
  });
  ...
  xit('sets width based on width attribute', function() {
    var menu = setup();
    openMenu(menu);
    var menuContainer = getOpenMenuContainer();
    var openMenuEl = menuContainer.children()[0];
    expect(openMenuEl.style.width).toBe(64);
  });
  ...
  it('closes on escape', inject(function($document, $mdConstant) {
    ...
    var openMenuEl = $document[0].querySelector('.md-open-menu');
